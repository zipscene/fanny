...
function validateNeuronConfigKey(val) {
	let keyRegex = /^[0-9]+$|^[0-9]+-[0-9]+$|^hidden$|^output$/;
	for (let key in val) {
		if (!keyRegex.test(key)) {
			let msg = 'Keys must be "hidden", "output", a layer number, ' +
...
// options can include: maxEpochs, progressInterval (in epochs), desiredError, maxNeurons (for cascade training)
...
//   this progress function returns false or a negative number, training is cancelled on the next iteration.
...
// - trainEpoch
// - trainData + cascade -> train
// - trainFile + cascade -> train
// - testOne
// - testData
// - run
