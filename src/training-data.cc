...
public:
    ...
    TDIOWorker(
        ...
        bool _isSave,
        bool _isFixed,
        unsigned int _decimalPoint) : Nan::AsyncWorker(callback), filename(_filename),
...
{
    if (info.Length() != 2)
        return Nan::ThrowError("Must have 2 arguments: input, output");
    if (!info[0]->IsArray() || !info[1]->IsArray())
        return Nan::ThrowError("Not an array");
    ...
    if (outputs->Length() != dataSetLength)
        return Nan::ThrowError("Input and output dataset sizes must match");
    if (!dataSetLength)
        return Nan::ThrowError("Dataset must be nonzero in size");
    ...
    for (unsigned int idx = 0; idx < dataSetLength; ++idx)
    {
        ...
        if (inputMaybeValue.IsEmpty() || outputMaybeValue.IsEmpty())
            return Nan::ThrowError("Invalid data");
        ...
        if (idx == 0)
        {
            ...
            if (!numInputNodes || !numOutputNodes)
                return Nan::ThrowError("Invalid data");
        ...
        if (inputRow.size() != numInputNodes || outputRow.size() != numOutputNodes)
            return Nan::ThrowError("Invalid data");
...
NAN_METHOD(TrainingData::readTrainFromFile)
{
    if (info.Length() < 2 || !info[0]->IsString())
        return Nan::ThrowError("Filename required");
...
NAN_METHOD(TrainingData::saveTrainToFixed)
{
    if (info.Length() < 3 || !info[0]->IsString() || !info[1]->IsNumber())
        return Nan::ThrowError("Filename and decimalPoint required");
    std::string filename = std::string(*Nan::Utf8String(info[0]));
...
{
    if (info.Length() != 2)
        return Nan::ThrowError("Must have 2 arguments: new_min, new_max");
...
{
    if (info.Length() != 2)
        return Nan::ThrowError("Must have 2 arguments: pos, legth");
    if (!info[0]->IsNumber() || !info[1]->IsNumber())
        return Nan::ThrowError("Arguments must be numbers");
